...
/**
 * The booking dialog itself — the branded panel and the Calendly iframe.
 *
 * Split out of BookingModal.tsx and loaded through next/dynamic. The trigger
 * that mounts in the root layout needs one document click listener; it does not
 * need the mark paths, the ISO artwork or the embed chrome on every route for
 * a dialog that opens on a click most visits never make.
 *
 * The fade lives here rather than in the parent, and that is the point of the
 * split rather than an accident of it: the overlay mounts at opacity 0 and
 * flips on the next frame, so the transition only runs if the flip happens
 * *after* this chunk has arrived and mounted. Owning `visible` here also means
 * the close animation can finish before the parent unmounts us — `onClose` is
 * called at the end of the fade, not the start.
 */
...
/* The Rem mark from RemAssist-Html/assets/booking-modal.js, drawn stroke-first behind the
...
/** Matches the fade-out in BookingModal.module.css. */
const FADE_MS = 280;

export default function BookingModalDialog({ onClose }: { onClose: () => void }) {
  ...
  /* Mount at opacity 0, then flip on the next frame so the transition runs —
     legacy booking-modal.js did the same with classList.add('is-visible')
  ...
  /* Fade out, then tell the parent to unmount us. Guarded because the backdrop
     and the close button can both fire before the timer lands, and two
     overlapping closes would call onClose twice. */
  function doClose() {
